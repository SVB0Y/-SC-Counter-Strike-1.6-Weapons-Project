...
namespace CSCZC4PROJECTILE
{

string DEFAULT_PROJ_NAME 	= "proj_csczc4";
string EXPLODE_SOUND    	= "csczsven/c4/boom.wav";//c4_explode1
string BEEP_SOUND       	= "csczsven/c4/c4_click.wav";//c4_click
float DEFAULT_TIMER     	= 35;//45 by default
int BOMB_RADIUS         	= 750;

class CCSCZC4 : ScriptBaseEntity
{
	...
	void Precache()
	{
		...
		//Sounds
		CSCZBASE::PrecacheSound( EXPLODE_SOUND );
		CSCZBASE::PrecacheSound( BEEP_SOUND );
	}
...
CCSCZC4@ PlantC4( entvars_t@ pevOwner, Vector vecOrigin, Vector vecAngles, float flTime, float flDmg, string sModel, const string& in szName = DEFAULT_PROJ_NAME )
{
	CBaseEntity@ cbeCSCZC4 = g_EntityFuncs.CreateEntity( szName );
	CCSCZC4@ CSCZC4 = cast<CCSCZC4@>( CastToScriptClass( cbeCSCZC4 ) );

	g_EntityFuncs.SetOrigin( CSCZC4.self, vecOrigin );
	g_EntityFuncs.SetModel( CSCZC4.self, sModel );
	g_EntityFuncs.DispatchSpawn( CSCZC4.self.edict() );
	@CSCZC4.pev.owner = CSCZBASE::ENT( pevOwner );

	CSCZC4.pev.velocity = g_vecZero;
	CSCZC4.pev.angles = vecAngles;

	CSCZC4.pev.dmgtime = g_Engine.time + flTime;
	CSCZC4.pev.dmg = flDmg;

	if( CSCZC4.pev.dmgtime - g_Engine.time <= 10.0f )
		CSCZC4.m_flBeepTime = 5;
	else
		CSCZC4.m_flBeepTime = flTime;

	CSCZC4.m_flNextBlink = g_Engine.time + 2;

	CSCZC4.SetTouch( TouchFunction( CSCZC4.PlantTouch ) );
...
void Register( const string& in szName = DEFAULT_PROJ_NAME )
{
	...
	g_CustomEntityFuncs.RegisterCustomEntity( "CSCZC4PROJECTILE::CCSCZC4", szName );
