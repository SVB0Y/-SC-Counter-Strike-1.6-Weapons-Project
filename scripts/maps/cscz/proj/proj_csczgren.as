...
namespace CSCZGRENADEPROJECTILE
{

string DEFAULT_PROJ_NAME 	= "proj_csczgrenade";
string BOUNCE_SOUND      	= "csczsven/he/bounce.wav";

class CCSCZGrenade : ScriptBaseMonsterEntity
{
	...
	void Precache()
	{
		...
		//Sounds
		CSCZBASE::PrecacheSound( BOUNCE_SOUND );
	}
...
CCSCZGrenade@ TossGrenade( entvars_t@ pevOwner, Vector vecStart, Vector vecVelocity, float flTime, float flDmg, string sModel, const string& in szName = DEFAULT_PROJ_NAME )
{
	CBaseEntity@ cbeCSCZGrenade = g_EntityFuncs.CreateEntity( szName );
	CCSCZGrenade@ CSCZGrenade = cast<CCSCZGrenade@>( CastToScriptClass( cbeCSCZGrenade ) );

	g_EntityFuncs.SetOrigin( CSCZGrenade.self, vecStart );
	g_EntityFuncs.SetModel( CSCZGrenade.self, sModel );
	g_EntityFuncs.DispatchSpawn( CSCZGrenade.self.edict() );

	CSCZGrenade.pev.velocity = vecVelocity;
	CSCZGrenade.pev.angles = Math.VecToAngles( CSCZGrenade.pev.velocity );
	@CSCZGrenade.pev.owner = CSCZBASE::ENT( pevOwner );

	CSCZGrenade.pev.dmg = flDmg;
	CSCZGrenade.pev.sequence = Math.RandomLong( 3, 6 );

	CSCZGrenade.SetTouch( TouchFunction( CSCZGrenade.BounceTouch ) );

	if( flTime < 0.1 )
	{
		CSCZGrenade.pev.nextthink = g_Engine.time;
		CSCZGrenade.pev.velocity = g_vecZero;
	}
	CSCZGrenade.pev.dmgtime = g_Engine.time + flTime;
...
void Register( const string& in szName = DEFAULT_PROJ_NAME )
{
	...
	g_CustomEntityFuncs.RegisterCustomEntity( "CSCZGRENADEPROJECTILE::CCSCZGrenade", szName );
