...
This script contains code integrates the Counter-Strike Condition Zero Project ONLY and if you paid money to access it, you have been scammed.
...
const string MoneySignSpr = CSCZBASE::MAIN_CSTRIKE_DIR + "640hud7.spr";
...
final class BuyMenuCVARS
{
	...
	HookReturnCode CSCZ_ClientPutInServer( CBasePlayer@ pPlayer )
	{
	...
	HookReturnCode CSCZ_PlayerPostThink( CBasePlayer@ pPlayer )
	{
		if( pPlayer is null ) //Null pointer checker
			return HOOK_CONTINUE;
	...
	HookReturnCode CSCZ_ClientSay( SayParameters@ pParams )
	{
		...
		else if( args.ArgC() == 3 && FirstArgChecker( args ) )
		{
			...
			{
				g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CSCZ BUYMENU] Invalid Item Type\n" );
				return HOOK_CONTINUE;
			}

			if( g_CSCZMenu.m_Items.length() > 0 )
			{
				for( uint i = 0; i < g_CSCZMenu.m_Items.length(); i++ )
				{
					if( szItemType + "_" + args.Arg(2).ToLowercase() == g_CSCZMenu.m_Items[i].EntityName )
					{
						bItemFound = true;
						szItemName = g_CSCZMenu.m_Items[i].EntityName;
						uiCost = g_CSCZMenu.m_Items[i].Cost;
						break;
					}
					else
						bItemFound = false;
				...
				if( bItemFound )
				{
					...
					{
						g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CSCZ BUYMENU] Not enough money to buy: " + szItemName + " - Cost: $" + uiCost + "\n" );
					}

					else if( pPlayer.HasNamedPlayerItem( szItemName ) !is null )
					{
						g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CSCZ BUYMENU] You already have this weapon: " + szItemName + "\n" );
					}
					else
					{ 
				...
				{
					g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, (args.Arg(2) == string_t()) ?  "[CSCZ BUYMENU] Invalid item\n" : "[CSCZ BUYMENU] Invalid item: " + args.Arg(2) + "\n" );
				}
			}
		}
				...
				if( pPlayer.m_hActiveItem.GetEntity() !is null )
				{
					...
					if( g_CSCZMenu.m_Items.length() > 0 )
					{
						for( uint i = 0; i < g_CSCZMenu.m_Items.length(); i++ )
						{
							...
							{
								if( szClassname == g_CSCZMenu.m_Items[i].EntityName )
								{
									bItemFound = true;
									szItemName = g_CSCZMenu.m_Items[i].EntityName;
									uiCost = g_CSCZMenu.m_Items[i].Cost;
							...
							else if( "ammo_" + szClassname.Split("_")[1] == g_CSCZMenu.m_Items[i].EntityName )
							{
								bItemFound = true;
								szItemName = g_CSCZMenu.m_Items[i].EntityName;
								uiCost = g_CSCZMenu.m_Items[i].Cost;
					...
					if( bItemFound )
					{
						...
						{
							g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CSCZ BUYMENU] Exhaustible weapon already at max ammo\n" );
						}
						else
						{ 
							if( uint(BuyMenu::BuyPoints[PlayerID( pPlayer )]) >= uiCost )
					...
					{
						g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CSCZ BUYMENU] Invalid Weapon or Ammo entity\n" );
					}
				}
			}
		}
	...
	void CSCZ_Buy( const CCommand@ args )
	{
		...
		else if( args.ArgC() == 3 )
		{
			...
			{
				g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTCONSOLE, "[CSCZ BUYMENU] Invalid Item Type\n" );
				return;
			}
			...
			if( g_CSCZMenu.m_Items.length() > 0 )
			{
				for( uint i = 0; i < g_CSCZMenu.m_Items.length(); i++ )
				{
					if( szItemType + "_" + args.Arg(2) == g_CSCZMenu.m_Items[i].EntityName )
					{
						bItemFound = true;
						szItemName = g_CSCZMenu.m_Items[i].EntityName;
						uiCost = g_CSCZMenu.m_Items[i].Cost;
				...
				if( bItemFound )
				{
					if( uint(BuyMenu::BuyPoints[PlayerID( pPlayer )]) <= 0 )
					{
						g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTCONSOLE, "[CSCZ BUYMENU] Not enough money to buy: " + szItemName + " - Cost: $" + uiCost + "\n" );
					}
					else if( pPlayer.HasNamedPlayerItem( szItemName ) !is null )
					{
						g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTCONSOLE, "[CSCZ BUYMENU] You already have this weapon: " + szItemName + "\n" );
					}
					else
					{ 
						if( uint(BuyMenu::BuyPoints[PlayerID( pPlayer )]) >= uiCost )
				...
				{
					g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTCONSOLE, (args.Arg(2) == string_t()) ?  "[CSCZ BUYMENU] Invalid item\n" : "[CSCZ BUYMENU] Invalid item: " + args.Arg(2) + "\n" );
				}
			}
		}
		else if( args.ArgC() == 2 )
				...
				if( pPlayer.m_hActiveItem.GetEntity() !is null )
				{
					...
					if( bItemFound )
					{
						...
						//Very tedious check to see if the player already has max ammo for a exhaustible weapon
							...
							pPlayer.m_rgAmmo( pPlayer.HasNamedPlayerItem( szClassname ).GetWeaponPtr().m_iPrimaryAmmoType ) == pPlayer.GetMaxAmmo( szClassname ) )
						{
							g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTCONSOLE, "[CSCZ BUYMENU] Exhaustible weapon already at max ammo\n" );
						}
						else
						{ 
							if( uint(BuyMenu::BuyPoints[PlayerID( pPlayer )]) >= uiCost )
					...
					{
						g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTCONSOLE, "[CSCZ BUYMENU] Invalid Weapon or Ammo entity\n" );
					}
				}
			}
		}
...
void RegisterBuyMenuCCVars()
{
	if( g_MaxMoney is null || g_MoneyPerScore is null || g_StartMoney is null ) //Check if they exist first
	{
		...
		@g_MaxMoney = CCVar( "bm_maxmoney", MaxMoney, "Maximum money the player can have", ConCommandFlag::AdminOnly ); //as_command CSCZ.bm_maxmoney
		@g_MoneyPerScore = CCVar( "bm_moneyperscore", MoneyPerScore, "Money the player will earn per score", ConCommandFlag::AdminOnly ); //as_command CSCZ.bm_moneyperscore
		@g_StartMoney = CCVar( "bm_startmoney", StartMoney, "Money the player will start once he joins the server", ConCommandFlag::AdminOnly ); //as_command CSCZ.bm_startmoney
	}
}
...
{
	BuyMenu::BuyMenuCVARS@ g_CSCZMenuHooks = @BuyMenu::BuyMenuCVARS();
	return g_CSCZMenuHooks.CSCZ_PlayerPostThink( pPlayer );
}
...
{
	BuyMenu::BuyMenuCVARS@ g_CSCZMenuHooks = @BuyMenu::BuyMenuCVARS();
	return g_CSCZMenuHooks.CSCZ_ClientPutInServer( pPlayer );
}
...
{
	BuyMenu::BuyMenuCVARS@ g_CSCZMenuHooks = @BuyMenu::BuyMenuCVARS();
	return g_CSCZMenuHooks.CSCZ_ClientSay( pParams );
}
...
{
	BuyMenu::BuyMenuCVARS@ g_CSCZMenuHooks = @BuyMenu::BuyMenuCVARS();
	g_CSCZMenuHooks.CSCZ_Buy( args );
}

CClientCommand _buy( "buy", "Opens the BuyMenu", @CSCZ_Buy );
...
void MoneyInit()
{
	...
	g_Game.PrecacheModel( CSCZBASE::MAIN_SPRITE_DIR + BuyMenu::MoneySignSpr );
...
final class BuyableItem
{
	...
	private BuyMenu::BuyMenuCVARS@ g_CSCZMenuHooks = @BuyMenu::BuyMenuCVARS();
	...
	{
		if( uint(BuyPoints[g_CSCZMenuHooks.PlayerID( pPlayer )]) < m_uiCost )
		{
			g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CSCZ BUYMENU] Not enough money to buy: " + m_szEntityName + " - Cost: $" +  m_uiCost + "\n" );
			return;
		}
		...
		if( pPlayer.HasNamedPlayerItem( m_szEntityName ) !is null )
		{
			...
			if( !(pPlayer.HasNamedPlayerItem( m_szEntityName ).iFlags() < 0) && pPlayer.HasNamedPlayerItem( m_szEntityName ).iFlags() & ITEM_FLAG_EXHAUSTIBLE != 0 )
			{
				if( pPlayer.GiveAmmo( pPlayer.HasNamedPlayerItem( m_szEntityName ).GetWeaponPtr().m_iDefaultAmmo, m_szEntityName, pPlayer.GetMaxAmmo( m_szEntityName ) ) != -1 )
				{
					...
					g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CSCZ BUYMENU] You bought ammo for: " + m_szEntityName + "\n" );
				}
				else
				{
					g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CSCZ BUYMENU] You already have max ammo for this item!\n" );
					return;
				}
			}
			else
			{
				g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CSCZ BUYMENU] You already have that item!\n" );
				return;
			}
			//KernCore end
		}
		...
		if( uint(BuyPoints[g_CSCZMenuHooks.PlayerID( pPlayer )]) >= m_uiCost )
		{
			int p = int(BuyPoints[g_CSCZMenuHooks.PlayerID( pPlayer )]);
			BuyPoints[g_CSCZMenuHooks.PlayerID( pPlayer )] = p - m_uiCost;
			g_CSCZMenuHooks.ShowPointsSprite( pPlayer );
		...
		{
			g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CSCZ BUYMENU] Not enough money - Cost: $" + m_uiCost + "\n");
			return;
		}
	}
}
