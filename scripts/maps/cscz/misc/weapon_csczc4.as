// Counter-Strike Condition Zero C4 Bomb
/* Model Credits
/ Model: Valve
/ Textures: Valve
/ Animations: Valve
...
// Models
string W_MODEL  	= "models/csczsven/wpn/c4/w_bp.mdl";
string V_MODEL  	= "models/csczsven/wpn/c4/v_c4.mdl";
string P_MODEL  	= "models/csczsven/wpn/c4/p_c4.mdl";
string C_MODEL  	= "models/csczsven/wpn/c4/w_c4.mdl";
// Sprites
string SPR_CAT  	= "misc/"; //Weapon category used to get the sprite's location
// Sounds
array<string> 		WeaponSoundEvents = {
					"csczsven/c4/c4_click.wav"
};
string PLANT_S   	= "csczsven/c4/c4_plant.wav"; //c4_plant
// Information
int MAX_CARRY   	= 1;
...
class weapon_csczc4 : ScriptBasePlayerWeaponEntity, CSCZBASE::WeaponBase, CSCZBASE::GrenadeWeaponExplode
{
	...
	void Precache()
	{
		...
		//Entities
		g_Game.PrecacheOther( CSCZC4PROJECTILE::DEFAULT_PROJ_NAME );
		//Sounds
		CSCZBASE::PrecacheSounds( WeaponSoundEvents );
		CSCZBASE::PrecacheSound( PLANT_S );
		//Sprites
	...
	void ShowC4Sprite( float flHoldTime )
	{
		...
		SpriteParams.spritename = CSCZBASE::MAIN_CSTRIKE_DIR + "640hud7.spr";
	...
	void PrimaryAttack()
	{
		...
		bool onBombZone = pCustom.GetKeyvalue( CSCZ_BOMBTARGET::INBOMB_KV ).GetInteger() == 1;

		if( !m_bStartedArming )
		{
			if( !onBombZone && CSCZ_BOMBTARGET::USE_BOMB_ZONES )
			{
		...
		{
			if( !onGround || (!onBombZone && CSCZ_BOMBTARGET::USE_BOMB_ZONES) )
			{
				if( onBombZone && CSCZ_BOMBTARGET::USE_BOMB_ZONES )
			...
			if( g_Engine.time > m_fArmedTime )
			{
				if( m_bStartedArming == true )
				{
					...
					auto pC4 = CSCZC4PROJECTILE::PlantC4( m_pPlayer.pev, m_pPlayer.pev.origin, Vector(0, 0, 0), TIMER, DAMAGE, C_MODEL );
...
string GetName()
{
	return "weapon_csczc4";
}

void Register()
{
	CSCZ_BOMBTARGET::Register();
	CSCZC4PROJECTILE::Register();
	CSCZBASE::RegisterCWEntityEX( "CSCZ_C4::", "weapon_csczc4", GetName(), GetName(), CSCZBASE::MAIN_CSTRIKE_DIR + SPR_CAT, AMMO_TYPE );
}
